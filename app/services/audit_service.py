...
def update_audit_status_closed_appdynamics(pid, serviceName, problemTitle, new_status):
    ...
    try:
        ...
        audit = Audit.query.filter_by(status ="IN_PROGRESS" ,serviceName = serviceName, problemTitle = problemTitle, problemEndAt=datetime.datetime.now(ist_timezone).strftime('%Y-%m-%d %H:%M:%S'))
        if audit:
            print(datetime.datetime.now(ist_timezone).strftime('%Y-%m-%d %H:%M:%S'),"audit found to update")
            audit.status = new_status
            db.session.commit()
            logger.info(f"Updated audit status for PID {pid} successfully")
            print(f"Audit with PID {pid} updated successfully")
            return {"message": f"Audit with PID {pid} updated successfully"}
        else:
            print("error")
            return {"error": f"Audit with PID {pid} not found"}, 404
